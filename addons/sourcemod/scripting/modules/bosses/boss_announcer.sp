...
public void Announcer_GetBossName(SaxtonHaleBase boss, char[] name, int length)
{
	strcopy(name, length, "Announcer");
}

public void Announcer_GetBossInfo(SaxtonHaleBase boss, char[] info, int length)
{
	StrCat(info, length, "\nHealth: Medium");
	StrCat(info, length, "\n ");
	StrCat(info, length, "\nAbilities");
...
public void Announcer_OnSpawn(SaxtonHaleBase boss)
{
	...
	char attribs[128]; Format(attribs, sizeof(attribs), "37 ; 0.0 ; 106 ; 0.0 ; 117 ; 0.0");
	weapon = boss.CallFunction("CreateWeapon", 525, "tf_weapon_revolver", 100, TFQual_Collectors, attribs);
	if (weapon > MaxClients)
	{
		SetEntProp(weapon, Prop_Send, "m_iClip1", 0);
		SetEntProp(client, Prop_Send, "m_iAmmo", 0, _, 2);
	}

	Format(attribs, sizeof(attribs), "2 ; 4.55 ; 252 ; 0.5 ; 259 ; 1.0");
	weapon = boss.CallFunction("CreateWeapon", 194, "tf_weapon_knife", 100, TFQual_Collectors, attribs);
	if (weapon > MaxClients)
		SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", weapon);
}

public void Announcer_GetModel(SaxtonHaleBase boss, char[] model, int length)
{
	strcopy(model, length, ANNOUNCER_MODEL);
}

public void Announcer_GetSound(SaxtonHaleBase boss, char[] sound, int length, SaxtonHaleSound soundtype)
{
	switch (soundtype)
	{
		case VSHSound_RoundStart:
			strcopy(sound, length, g_strAnnouncerRoundStart[GetRandomInt(0,sizeof(g_strAnnouncerRoundStart)-1)]);
		case VSHSound_Win:
			strcopy(sound, length, g_strAnnouncerWin[GetRandomInt(0,sizeof(g_strAnnouncerWin)-1)]);
		case VSHSound_Lose:
			strcopy(sound, length, g_strAnnouncerLose[GetRandomInt(0,sizeof(g_strAnnouncerLose)-1)]);
		case VSHSound_Lastman:
			strcopy(sound, length, g_strAnnouncerLastMan[GetRandomInt(0,sizeof(g_strAnnouncerLastMan)-1)]);
		case VSHSound_Backstab:
			strcopy(sound, length, g_strAnnouncerBackStabbed[GetRandomInt(0,sizeof(g_strAnnouncerBackStabbed)-1)]);
	}
}

public void Announcer_GetSoundKill(SaxtonHaleBase boss, char[] sound, int length, TFClassType class)
{
	strcopy(sound, length, g_strAnnouncerKill[GetRandomInt(0,sizeof(g_strAnnouncerKill)-1)]);
}

public void Announcer_GetMusicInfo(SaxtonHaleBase boss, char[] sound, int length, float &time)
{
	strcopy(sound, length, ANNOUNCER_THEME);
	time = 67.5;
}
...
{
	int client = boss.iClient;
	int playercount = SaxtonHale_GetAliveAttackPlayers();
	//Add ammo to primary weapon
	int primary = GetPlayerWeaponSlot(client, WeaponSlot_Primary);
	if (IsValidEntity(primary))
	{
		int clip = 2 + RoundToFloor(playercount / 8.0);
		if (boss.bSuperRage)
			clip *= 2;
...
public Action Announcer_OnAttackDamage(SaxtonHaleBase boss, int victim, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	if (TF2_GetItemSlot(index, TF2_GetPlayerClass(client)) != WeaponSlot_Primary)
		return Plugin_Continue;
	...
	SaxtonHaleBase boss_victim = SaxtonHaleBase(victim);
	if (boss_victim.bValid)
		boss_victim.DestroyAllClass();
	boss_victim.CreateClass("AnnouncerMinion");
	...
	char message[128];
	Format(message, sizeof(message), "A%s %s was hit and will switch teams!", (class == TFClass_Engineer ? "n" : ""), g_strClassName[class]);
	Announcer_ShowAnnotation(victim, message, 6.0);
	
	EmitSoundToClient(client, SOUND_BACKSTAB);
...
public Action Announcer_OnAttackBuilding(SaxtonHaleBase boss, int building, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	TFObjectType type = TF2_GetBuildingType(building);
	TFObjectMode mode = TF2_GetBuildingMode(building);
	
	char message[128];
	Format(message, sizeof(message), "A %s was hit and has switched teams!", g_strBuildingName[type][mode]);
	Announcer_ShowAnnotation(building, message, 3.0);
	
	TF2_SetBuildingTeam(building, TF2_GetClientTeam(client), client);
	EmitSoundToClient(client, g_strAnnouncerHitBuilding[GetRandomInt(0, sizeof(g_strAnnouncerHitBuilding)-1)]);
	damage = 0.0;
	return Plugin_Changed;
}
